...
def download_amazon_books(proj_dir):
    ...
    # Sanity check
    print("\nSanity check:")
    run_command(["gunzip", "-c", str(output_file), "|", "head", "-n", "2"], cwd=output_dir)
...
def download_movielens(proj_dir):
    ...
    run_command(["unzip", "-q", str(output_file)], cwd=output_dir)
    
    # Move the extracted directory to tmp_ml25m
    extracted_dir = output_dir / "ml-25m"
    if extracted_dir.exists():
        run_command(["mv", str(extracted_dir), str(tmp_dir)], cwd=output_dir)
...
def download_steam(proj_dir):
    ...
    # Sanity check
    print("\nSanity check:")
    run_command(["wc", "-l", str(output_file)], cwd=output_dir)
    run_command(["head", "-n", "3", str(output_file)], cwd=output_dir)
...
def main():
    ...
    run_command(["du", "-sh", str(Path(args.proj_dir) / "data" / "raw" / "*")])
    run_command(["df", "-h", "/workspace"])
